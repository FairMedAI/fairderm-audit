...
Adding 290 synthetic dark-skin melanoma images increased overall test AUROC slightly from 0.634 to 0.640 [$\Delta$+0.006] relative to fine-tuned, but reduced Dark AUROC from 0.547 to 0.481. Light--Dark gap widened to $-0.291$. Paired Dark delta $-0.066$ [95\% CI $-0.151,0.008$], one-tailed $p=0.962$ [$H_1$: $\Delta>0$, $p=$\,prop(bootstrap $\Delta \leq 0$)]. Sensitivity was 0.600 Light vs 0.500 Dark [gap $-0.100$] at Youden-J threshold [stage-specific].
...
Results are summarized in Table~\ref{tab:ablation} and visualized in Figure~\ref{fig:ablation_tradeoff}. The ablation reveals a non-monotonic relationship between synthetic data quantity and Dark subgroup performance: Dark AUROC was 0.5438 at 0$\times$, 0.5750 at 2$\times$, 0.5344 at 5$\times$, and 0.4812 at 10$\times$. Overall test AUROC was 0.6848, 0.7063, 0.6742, and 0.6433, respectively. Validation AUROC peaked at 2$\times$, so no single multiplier was uniformly optimal.
